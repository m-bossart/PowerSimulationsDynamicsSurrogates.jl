...
@testset "Test serialization with surrogates in the system" begin
    ...
    try
        input = [0.2; 0.4]
        model = Lux.Chain(WrappedFunction(x -> x .* sin(1.5)), Lux.Dense(2, 5, NNlib.relu), Lux.Dense(5, 2))
        p1, st1 = Lux.setup(rng, model)
        sys = System("test/data_tests/9BusSystem.json")
        g = collect(get_components(ThermalStandard, sys))[1]
        set_ext!(g,  Dict{String, Any}("model_path" => "", "model" => model, "ps"=> p1, "st" => st1))  #Store the Flux model in the ext field of a component
        ...
        function to_json_with_surrogates(sys, full_path)
            ...
            set_ext!(g,  Dict{String, Any}("model_path" => joinpath(dir, "surrogate_models", get_name(g)), "model" => nothing))   
        ...
        function deserialize_with_surrogates(full_path)
            ...
            set_ext!(g,  Dict{String, Any}("model_path" => nothing, "model" => model, "ps"=>p, "st" =>st))   
            return sys
        end 
