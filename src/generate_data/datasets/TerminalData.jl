mutable struct TerminalData <: SurrogateDataset
    ...
    stable::Bool
    solve_time::Float64
    device_terminal_data::Dict{String, Dict{Symbol, AbstractArray}} #string is device name, "symbol is :vr, :vi, :ir, :ii, arrays are the data
end
...
function fill_surrogate_data!(
    ...
    for (device_name, orientation_details) in device_details
        all_components_with_name = PSY.get_components_by_name(PSY.Component, sys, device_name) 
        exclude_dynamic_injectors =  filter!(x -> !(typeof(x) <: PSY.DynamicInjection), all_components_with_name)
        @assert length(exclude_dynamic_injectors) == 1 
        device = exclude_dynamic_injectors[1]
        _fill_terminal_data!(terminal_data_dict, results, device, save_indices, orientation_details, data_collection_params, sys)
    end 
...
function _fill_terminal_data!(terminal_data_dict, results, device, save_indices, orientation_details, data_collection_params, sys)
    @error "Function _fill_terminal_data! not defined for type $(typeof(device)) "
    return 
end 

function _fill_terminal_data!(terminal_data_dict, results, device::S, save_indices, orientation_details, data_collection_params, sys) where {S <: PSY.StaticInjection}
    data_dict =  Dict{Symbol, AbstractArray}()
    device_name = PSY.get_name(device)
    dyn_source = PSY.get_dynamic_injector(device)
    ...
    if dyn_source === nothing
        ...
        data_dict[:ir] = scale .* PSID.get_source_real_current_series(results, device_name)[2][save_indices]
        data_dict[:ii] = scale .* PSID.get_source_imaginary_current_series(results, device_name)[2][save_indices]
    else
        data_dict[:ir] = scale .* PSID.get_real_current_series(results, device_name)[2][save_indices]
        data_dict[:ii] = scale .* PSID.get_imaginary_current_series(results, device_name)[2][save_indices]
    end
...
function _fill_terminal_data!(terminal_data_dict, results, device::PSY.Arc, save_indices, orientation_details, data_collection_params, sys)
    data_dict =  Dict{Symbol, AbstractArray}()
    corresponding_branches = collect(PSY.get_components(x -> PSY.get_arc(x) == device, PSY.Branch, sys))   
    device_name = PSY.get_name(device)
    ...
    V = PSID.get_voltage_magnitude_series(results, bus_number_voltage_measurement)[2][save_indices]
    θ = PSID.get_voltage_angle_series(results, bus_number_voltage_measurement)[2][save_indices]
...
function generate_empty_plot(T::Type{TerminalData})
    p = PlotlyJS.make_subplots(
        rows = 2,
        cols = 3,
        specs = [PlotlyJS.Spec() PlotlyJS.Spec() PlotlyJS.Spec(); PlotlyJS.Spec() PlotlyJS.Spec() PlotlyJS.Spec()],
...
function add_data_trace!(p, data::TerminalData;  name = "")
    for (device_name, device_data_dict) in data.device_terminal_data
        ...
        PlotlyJS.add_trace!(p, PlotlyJS.scatter(; x = data.tsteps, y = P, name = string(device_name, name)), row = 1, col = 3)
        PlotlyJS.add_trace!(
            p,
        ...
        PlotlyJS.add_trace!(p, PlotlyJS.scatter(; x = data.tsteps, y = Q, name =  string(device_name, name)), row = 2, col = 3)
    end 
    return p
end