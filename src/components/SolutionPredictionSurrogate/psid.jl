...
function _allocate_weights_and_biases!(
    ext::Dict{String, Any},
    static_device::SolutionPredictionSurrogate,
)
    Ws = []
    bs = []
...
get_W_nn(wrapper::PSID.StaticWrapper{SolutionPredictionSurrogate}) = wrapper.ext["nn"]["W"]
get_b_nn(wrapper::PSID.StaticWrapper{SolutionPredictionSurrogate}) = wrapper.ext["nn"]["b"]
...
function PSID.device!(
    ...
    mdl_solution_prediction_surrogate!(
        ...
        device,
        t,
    )  #Device (the static wrapper) has past values
    return
end
...
function mdl_solution_prediction_surrogate!(
    ...
    x_prime =
        _calculate_input_features(static_device, past_values_dict, voltage_r, voltage_i, t)
...
function _forward_pass_nn(wrapper::PSID.StaticWrapper{SolutionPredictionSurrogate}, x)
    W_index = 1
    b_index = 1
    W = get_W_nn(wrapper)
    b = get_b_nn(wrapper)
...
function PSID._add_callback!(
    ...
    f =
        (u, t, integrator) -> begin
            ...
            past_values_dict = PSID.get_ext(static_device)["past_values"]
            x_prime = _calculate_input_features(
                static_device,
                past_values_dict,
                voltage_r,
                voltage_i,
            ...
            y = _forward_pass_nn(static_device, x)
            y_prime = _target_scale_inverse(static_device, y)
            ir, ii = _calculate_current_from_output(
                static_device,
                y_prime,
                voltage_r,
                voltage_i,
